...
@section('content')
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8 cart-container">
        <!-- Breadcrumbs -->
        @php
                                ...
                                <div class="flex flex-col md:flex-row md:items-center gap-4">
                                    ...
                                    <div class="md:w-32">
                                        <form action="{{ route('cart.update', $item['cart_item_id']) }}" method="POST"
                                            ...
                                            data-url="{{ route('cart.update', $item['cart_item_id']) }}">
                                            @csrf
                                            @method('PUT')
                                            <div class="flex items-center justify-center">
                                                <div class="flex items-center border border-gray-300 rounded-lg">
                                                    <button type="button"
                                                        class="quantity-decrement px-3 py-2 text-gray-600 hover:text-gray-900 disabled:text-gray-300 disabled:cursor-not-allowed"
                                                        data-product-id="{{ $item['product_id'] }}"
                                                    ...
                                                    <input type="number" name="quantity" value="{{ $item['quantity'] }}"
                                                        ...
                                                        data-product-id="{{ $item['product_id'] }}"
                                                        class="quantity-input w-16 text-center border-x border-gray-300 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                                                    <button type="button"
                                                        class="quantity-increment px-3 py-2 text-gray-600 hover:text-gray-900 disabled:text-gray-300 disabled:cursor-not-allowed"
                                                        data-product-id="{{ $item['product_id'] }}"
                                    ...
                                    <div class="md:w-32 flex flex-col items-center space-y-2">
                                        ...
                                        <form action="{{ route('cart.remove', $item['cart_item_id']) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="cart-remove-item text-sm text-red-600 hover:text-red-800 transition-colors">
                                                Remove
                                            </button>
                                        </form>
                                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-24">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between text-gray-600">
                                <span>Subtotal</span>
                                <span id="cart-subtotal" class="font-medium">TK{{ number_format($subtotal, 2) }}</span>
                            </div>
                            <div class="flex justify-between text-gray-600">
                                <span>Shipping</span>
                        ...
                        <a href="{{ route('customer.checkout.index') }}"
                            class="checkout-btn block w-full bg-green-600 text-white text-center py-3 rounded-lg hover:bg-green-700 transition-colors font-medium mt-6">
                            Proceed to Checkout
                        </a>
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!window.cartManager && typeof CartManager !== 'undefined') {
                window.cartManager = new CartManager();
            }

            // Update checkout button state based on initial cart count
            const initialCount = {{ $itemCount }};
            if (window.cartManager && typeof window.cartManager.updateCheckoutButtons === 'function') {
                window.cartManager.updateCheckoutButtons(initialCount);
            }
        });
    </script>
@endpush
